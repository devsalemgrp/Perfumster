...
const createProduct = (req, res) => {
  ...
  const imageFilePath = `${IMAGE_PATH + req.file.filename}`;
...
const deleteProduct = (req, res) => {
  ...
  database.db.query(
    `SELECT image FROM ${PRODUCTS_PAGE_TABLE} WHERE id = ?`,
    [id],
    (err, rows) => {
      if (err) {
      ...
      database.db.query(
        `DELETE FROM ${PRODUCTS_PAGE_TABLE} WHERE id = ?`,
        [id],
        (err, result) => {
          if (err) {
  ...
  database.db.query(
    ...
    (err, results) => {
      ...
      if (newImage) {
        ...
        const newImagePath = `${IMAGE_PATH + newImage.filename}`;
        database.db.query(
          `UPDATE ${PRODUCTS_PAGE_TABLE} SET name = ?, price = ?, description = ?, status = ?, image = ? , subscriptionCategory=? , category=? WHERE id = ?`,
          [
            name,
            price,
            description,
